...
import com.pillhelper.DAO.CVSManager;
...
import com.pillhelper.DAO.DatabaseHelper;
import com.pillhelper.DAO.Fonte;
import com.pillhelper.DAO.Medication;
import com.pillhelper.DAO.Profile;
import com.pillhelper.DAO.LongTerm.DAOOldMedication;
import com.pillhelper.DAO.LongTerm.OldMedication;
import com.pillhelper.medicationmeasures.MedicationListFragment;
import com.pillhelper.fonte.FontesPagerFragment;
import com.pillhelper.intro.MainIntroActivity;
import com.pillhelper.specifications.MedicationFragment;
import com.pillhelper.utils.CustomExceptionHandler;
import com.pillhelper.utils.FileChooserDialog;
import com.pillhelper.utils.ImageUtil;
import com.pillhelper.utils.MusicController;
...
import static com.pillhelper.DAO.DAORecord.DOSE;
...
public class MainActivity extends AppCompatActivity {
    ...
    public static String longTermPrescription = "LongTermPrescription";
    public static String DOSE = "Measures";
    public static String PROFILE = "Profile";
    public static String HEALTHTRACKING = "BodyTracking";
    public static String BODYTRACKINGDETAILS = "BodyTrackingDetail";
    public static String ABOUT = "About";
    public static String SETTINGS = "Settings";
    public static String MEDICATION = "Medication";
    public static String MEDICATIONDETAILS = "Medicationdetails";
    ...
    private LongTermFragment mpLongTermFrag = null;
    private DoseFragment mpDoseFrag = null;
    private ProfileFragment mpProfileFrag = null;
    private MedicationFragment mpMedicationFrag = null;
    private SettingsFragment mpSettingFrag = null;
    private MedicationListFragment mpBodyPartListFrag = null;
    ...
    private String mCurrentMedication = "";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            if (mpLongTermFrag == null) mpLongTermFrag = LongTermFragment.newInstance(longTermPrescription, 4);
            if (mpDoseFrag == null) mpDoseFrag = DoseFragment.newInstance(DOSE, 5);
            ...
            if (mpMedicationFrag == null) mpMedicationFrag = MedicationFragment.newInstance(MEDICATION, 7);
            if (mpBodyPartListFrag == null)
                mpBodyPartListFrag = MedicationListFragment.newInstance(HEALTHTRACKING, 9);
        } else {
            mpFontesPagerFrag = (FontesPagerFragment) getSupportFragmentManager().getFragment(savedInstanceState, FONTESPAGER);
            mpLongTermFrag = (LongTermFragment) getSupportFragmentManager().getFragment(savedInstanceState, longTermPrescription);
            mpDoseFrag = (DoseFragment) getSupportFragmentManager().getFragment(savedInstanceState, DOSE);
            ...
            mpMedicationFrag = (MedicationFragment) getSupportFragmentManager().getFragment(savedInstanceState, MEDICATION);
            mpBodyPartListFrag = (MedicationListFragment) getSupportFragmentManager().getFragment(savedInstanceState, HEALTHTRACKING);
        }
        ...
        if (DatabaseHelper.DATABASE_VERSION >= 15 && !mMigrationBD15done) {
            DAOOldMedication mDbOldLongTerm = new DAOOldMedication(this);
            DAOMedication lDAOMedication = new DAOMedication(this);
            if (mDbOldLongTerm.tableExists()) {
                DAOLongTerm mDbLongTerm = new DAOLongTerm(this);
                List<OldMedication> mList = mDbOldLongTerm.getAllRecords();
                for (OldMedication record : mList) {
                    String medicationName = "";
                    Medication m = lDAOMedication.getMedication(record.getMedication());
                    medicationName = record.getMedication();
                    if (m != null) { // if the medication exists
                        if (m.getType() == DAOMedication.TYPE_FONTE) { // if it is not a LongTermPrescription type
                            medicationName = medicationName + "-LongTermPrescription"; // add a suffix to
                        }
                    }

                    mDbLongTerm.addLongTermRecord(record.getDate(), "00:00", medicationName, record.getDose(), record.getDuration(), record.getProfil());
                }
                mDbOldLongTerm.dropTable();

                DAOFonte mDbFonte = new DAOFonte(this);
                List<Fonte> mFonteList = mDbFonte.getAllMedicationRecords();
                for (Fonte record : mFonteList) {
                    mDbFonte.updateRecord(record); // Automatically update record Type
                }
                ArrayList<Medication> medicationList = lDAOMedication.getAllMedicationArray();
                for (Medication record : medicationList) {
                    lDAOMedication.updateMedication(record); // Reset all the fields on specifications.
                }
            }
        ...
        /* Erstellung des Speicherungsfiles*/

        File folder = new File(Environment.getExternalStorageDirectory() + "/pillhelper");
        boolean success = true;
        if (!folder.exists()) {
            success = folder.mkdir();
        }
        if (success) {
            folder = new File(Environment.getExternalStorageDirectory() + "/pillhelper/crashreport");
            success = folder.mkdir();
        }
        ...
        if (folder.exists()) {
            if (!(Thread.getDefaultUncaughtExceptionHandler() instanceof CustomExceptionHandler)) {
                Thread.setDefaultUncaughtExceptionHandler(new CustomExceptionHandler(
                    Environment.getExternalStorageDirectory() + "/pillhelper/crashreport"));
            }
        }
        ...
        dataList.add(new DrawerItem(this.getResources().getString(R.string.menu_Prescription), R.drawable.ic_filter_vintage_white_24dp, true));
        dataList.add(new DrawerItem(this.getResources().getString(R.string.MedicationsLabel), R.drawable.ic_filter_vintage_white_24dp, true));
        dataList.add(new DrawerItem(this.getResources().getString(R.string.HealthMenuLabel), R.drawable.ic_scale, true));
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (getLongTermFragment().isAdded())
            getSupportFragmentManager().putFragment(outState, longTermPrescription, mpLongTermFrag);
     //   if (getDoseFragment().isAdded())
     //       getSupportFragmentManager().putFragment(outState, DOSE, mpDoseFrag);
        if (getProfileFragment().isAdded())
            getSupportFragmentManager().putFragment(outState, PROFILE, mpProfileFrag);
     //   if (getMedicationFragment().isAdded())
      //      getSupportFragmentManager().putFragment(outState, MEDICATION, mpMedicationFrag);
        ...
        if (getBodyPartFragment().isAdded())
            getSupportFragmentManager().putFragment(outState, HEALTHTRACKING, mpBodyPartListFrag);
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.import_database:
                ...
                fileChooserDialog.chooseDirectory(Environment.getExternalStorageDirectory() + "/pillhelper/export");
                return true;
            case R.id.action_deleteDB:
                ...
                deleteDbBuilder.setPositiveButton(getActivity().getResources().getText(R.string.global_yes), (dialog, which) -> {
                    ...
                    DAOMedication mDbMedication = new DAOMedication(getActivity());
                    // wir kriegen die erste ID der Liste
                    List<Medication> lList2 = mDbMedication.getAllMedicationArray();
                    for (int i = 0; i < lList2.size(); i++) {
                        Medication mTemp = lList2.get(i);
                        mDbMedication.delete(mTemp.getId());
                    }
    ...
    private void showFragment(String pFragmentName, boolean addToBackStack) {
        ...
        } else if (pFragmentName.equals(longTermPrescription)) {
            ft.replace(R.id.fragment_container, getLongTermFragment(), longTermPrescription);
        } else if (pFragmentName.equals(DOSE)) {
            ft.replace(R.id.fragment_container, getDoseFragment(), DOSE);
        } else if (pFragmentName.equals(SETTINGS)) {
            ft.replace(R.id.fragment_container, getSettingsFragment(), SETTINGS);
        } else if (pFragmentName.equals(MEDICATION)) {
            ft.replace(R.id.fragment_container, getMedicationFragment(), MEDICATION);
        } else if (pFragmentName.equals(HEALTHTRACKING)) {
            ft.replace(R.id.fragment_container, getBodyPartFragment(), HEALTHTRACKING);
    ...
    private void savePhotoProfile(String path) {
        mCurrentProfile.setPhoto(path);// Der Photo Path wird eingefügt
        mDbProfils.updateProfile(mCurrentProfile);
    }

    public String getCurrentMedication() {
        return mCurrentMedication;
    }

    public void setCurrentMedication(String newMedication) {
        mCurrentMedication = newMedication;
    }
    ...
    private LongTermFragment getLongTermFragment() {
        if (mpLongTermFrag == null)
            mpLongTermFrag = (LongTermFragment) getSupportFragmentManager().findFragmentByTag(longTermPrescription);
        if (mpLongTermFrag == null) mpLongTermFrag = LongTermFragment.newInstance(longTermPrescription, 2);

        return mpLongTermFrag;
    }

    private DoseFragment getDoseFragment() {
        if (mpDoseFrag == null)
            mpDoseFrag = (DoseFragment) getSupportFragmentManager().findFragmentByTag(DOSE);
        if (mpDoseFrag == null) mpDoseFrag = DoseFragment.newInstance(DOSE, 5);
    ...
    private MedicationFragment getMedicationFragment() {
        if (mpMedicationFrag == null)
            mpMedicationFrag = (MedicationFragment) getSupportFragmentManager().findFragmentByTag(MEDICATION);
        if (mpMedicationFrag == null) mpMedicationFrag = MedicationFragment.newInstance(MEDICATION, 7);
        return mpMedicationFrag;
    }

    private MedicationListFragment getBodyPartFragment() {
        if (mpBodyPartListFrag == null)
            mpBodyPartListFrag = (MedicationListFragment) getSupportFragmentManager().findFragmentByTag(HEALTHTRACKING);
        if (mpBodyPartListFrag == null)
            mpBodyPartListFrag = MedicationListFragment.newInstance(HEALTHTRACKING, 9);
        ...
        public void onItemClick(AdapterView parent, View view, int position, long id) {
            ...
            switch (position) {
                ...
                case 1:
                    ...
                    setTitle(getResources().getText(R.string.menu_Prescription));
                    break;
                case 2:
                    showFragment(MEDICATION);
                    setTitle(getResources().getText(R.string.MedicationsLabel));
                    break;
                case 3:
                    showFragment(DOSE);
                    setTitle(getResources().getText(R.string.HealthMenuLabel));
                    break;
                case 4:
                    showFragment(HEALTHTRACKING);
