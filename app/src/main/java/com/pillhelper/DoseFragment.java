...
import com.pillhelper.DAO.DAODose;
...
import com.pillhelper.medicationmeasures.MedicationDetailsFragment;
import com.pillhelper.utils.EditableInputView.EditableInputView;
import com.pillhelper.utils.EditableInputView.EditableInputViewWithDate;
import com.pillhelper.utils.Gender;
...
public class DoseFragment extends Fragment {
    ...
    private DAODose mDoseDb = null;
    private DAOMedicationMeasure mDbMedicationMeasure = null;
    private DAOProfil mDb = null;
    private AdapterView.OnClickListener showDetailsFragment = v -> {
        int bodyPartID = MedicationDose.DOSE;
        switch (v.getId()) {
            case R.id.weightDetailsButton:
                bodyPartID = MedicationDose.DOSE;
                break;
            case R.id.fatDetailsButton:
                bodyPartID = MedicationDose.FAT;
                break;
            case R.id.musclesDetailsButton:
                bodyPartID = MedicationDose.MUSCLES;
                break;
            case R.id.waterDetailsButton:
                bodyPartID = MedicationDose.WATER;
                break;
        }

        MedicationDetailsFragment fragment = MedicationDetailsFragment.newInstance(bodyPartID, false);
        ...
        try {
            switch (view.getId()) {
                case R.id.weightInput:
                    ...
                    mDbMedicationMeasure.addMedicationMeasure(v.getDate(), MedicationDose.DOSE, weightValue, getProfil().getId());
                    break;
                case R.id.fatInput:
                    float fatValue = Float.parseFloat(v.getText());
                    mDbMedicationMeasure.addMedicationMeasure(v.getDate(), MedicationDose.FAT, fatValue, getProfil().getId());
                    break;
                case R.id.musclesInput:
                    float musclesValue = Float.parseFloat(v.getText());
                    mDbMedicationMeasure.addMedicationMeasure(v.getDate(), MedicationDose.MUSCLES, musclesValue, getProfil().getId());
                    break;
                case R.id.waterInput:
                    float waterValue = Float.parseFloat(v.getText());
                    mDbMedicationMeasure.addMedicationMeasure(v.getDate(), MedicationDose.WATER, waterValue, getProfil().getId());
                    break;
            }
     ...
     */
    public static DoseFragment newInstance(String name, int id) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.tab_dose, container, false);
        ...
        mDoseDb = new DAODose(view.getContext());
        mDbMedicationMeasure = new DAOMedicationMeasure(view.getContext());
    ...
    private void refreshData() {
        ...
        if (fragmentView != null) {
            if (getProfil() != null) {
                MedicationDoseMeasure lastWeightValue = null;
                MedicationDoseMeasure lastWaterValue = null;
                MedicationDoseMeasure lastFatValue = null;
                MedicationDoseMeasure lastMusclesValue = null;

                if (getProfil() != null) {
                    lastWeightValue = mDbMedicationMeasure.getLastMedicationMeasures(MedicationDose.DOSE, getProfil());
                    lastWaterValue = mDbMedicationMeasure.getLastMedicationMeasures(MedicationDose.WATER, getProfil());
                    lastFatValue = mDbMedicationMeasure.getLastMedicationMeasures(MedicationDose.FAT, getProfil());
                    lastMusclesValue = mDbMedicationMeasure.getLastMedicationMeasures(MedicationDose.MUSCLES, getProfil());
                }

                if (lastWeightValue != null) {
                    weightEdit.setText(String.valueOf(lastWeightValue.getMedicationMeasure()));
                    // update IMC
                    ...
                    if (size == 0) {
                        ...
                        float imcValue = calculateImc(lastWeightValue.getMedicationMeasure(), size);
                        imcText.setText(String.format("%.1f", imcValue));
                        imcRank.setText(getImcText(imcValue));
                        if (lastFatValue!=null) {
                            double ffmiValue = calculateFfmi(lastWeightValue.getMedicationMeasure(), size, lastFatValue.getMedicationMeasure());
                ...
                if (lastWaterValue != null)
                    waterEdit.setText(String.valueOf(lastWaterValue.getMedicationMeasure()));
                else
                    waterEdit.setText("-");

                if (lastFatValue != null)
                    fatEdit.setText(String.valueOf(lastFatValue.getMedicationMeasure()));
                else
                    fatEdit.setText("-");

                if (lastMusclesValue != null)
                    musclesEdit.setText(String.valueOf(lastMusclesValue.getMedicationMeasure()));
                else
                    musclesEdit.setText("-");
            }
        }
    ...
    private void showDeleteDialog(final long idToDelete) {
        ...
        new SweetAlertDialog(getContext(), SweetAlertDialog.WARNING_TYPE)
            ...
            .setConfirmClickListener(sDialog -> {
                mDbMedicationMeasure.deleteMeasure(idToDelete);
                refreshData();
                // Info
