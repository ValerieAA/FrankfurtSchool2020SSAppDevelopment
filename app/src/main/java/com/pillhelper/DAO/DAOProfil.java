...
public class DAOProfil extends DAOBase {
    ...
    public void deleteProfil(long id) {
        ...
        DAODose mDoseDb;
        mDoseDb = new DAODose(null); // null car a ce moment le DatabaseHelper est cree depuis bien longtemps.

        List<ProfileDose> valueList = (List<ProfileDose>) mDoseDb.getDoseList(getProfil(id));
        for (int i = 0; i < valueList.size(); i++) {
            mDoseDb.deleteMeasure(valueList.get(i).getId());
        }

        // Medicationmeasures

        DAOMedicationMeasure mMedicationDb;
        mMedicationDb = new DAOMedicationMeasure(null);
        List<MedicationDoseMeasure> medicationDoseMeasuresList = mMedicationDb.getMedicationMeasuresList(getProfil(id));
        for (int i = 0; i < medicationDoseMeasuresList.size(); i++) {
            mMedicationDb.deleteMeasure(medicationDoseMeasuresList.get(i).getId());
        }
